...
namelist = Namelist({
    ...
    'sat_vapor_pres_nml': {
        'do_simple':True,
        'tcmin_simple':  -600,    #Changed tcmin to tcmin_simple and expanded low bound   
        'tcmax_simple': 2000,     #Changed tcmax to tcmax_simple and expanded upper bound
    },
...
#     },
    ...
    'astronomy_nml': {
        'ecc':0.0935,
        'obliq':60,                            #Varied from original at 25.19 to 0, 15, 30, 45 and 60
...
if __name__=="__main__":
    ...
    conv_schemes = ['dry'] #Ran only dry simulations 'none' was also an option

    depths = [0.5]    #Selected 0.5 from a choice of 0.1, 0.5 and 1.0
    ...
    for conv in conv_schemes:
        for depth_val in depths:
            for per_value in pers:
                exp_name = f'grey_mars_full_{timestamp}_obliq{60}' #Changed experiment name and added timestamp and obliquity
